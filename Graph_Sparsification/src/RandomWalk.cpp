...
vector<Edge> RandomWalk::get_sampled_graph(vector<vector<int> > &paths, int sampled_size, random_walk_type t, double p) {
	vector<int> nodes;
	vector<Edge> ret;
	switch(t) {
...
vector<Edge> RandomWalk::graph_from_sampled_points(vector<vector<int> > &paths, vector<int> &nodes) {
	vector<Edge> ret;
	int n = paths.size();
...
vector<int> RandomWalk::rw_sampled_points(vector<vector<int> > &paths, int sampled_size) {
	int n = paths.size();
	srand (time(NULL));
	int s = rand() % n;
	// start from s
...
vector<int> RandomWalk::rwj_sampled_points(vector<vector<int> > &paths, int sampled_size, double jump_prob ) {
	int n = paths.size();
	srand(time(NULL));
	int s = rand() % n;
	int iterations = 0;
